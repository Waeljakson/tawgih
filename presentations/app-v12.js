...
async function submitCustomRequest(){const request=customRequestPayload();if(!request.title||!request.objective)return showStatus(el.customRequestStatus,"اكتب عنوان البرنامج والهدف المطلوب.",true);if(!el.customWhatsappConfirmed.checked)return showStatus(el.customRequestStatus,"تواصل عبر واتساب ثم فعّل الإقرار.",true);el.submitCustomRequestButton.disabled=true;try{if(SCHOOL_EDITION){if(!window.MishkatBubbleStore)throw new Error('الخدمة غير جاهزة حاليًا.');await window.MishkatBubbleStore.savePresentationModel({title:request.title,topic:CATEGORY_LABELS[request.category]||request.category,targetGroup:GENDER_LABELS[request.gender]||request.gender,presentationData:{kind:'custom_request',...request},theme:'custom-request',slidesCount:request.slides,status:'requested'});showStatus(el.customRequestStatus,"تم تسجيل طلب العرض في قاعدة المدرسة.");toast("تم تسجيل طلب العرض");el.customTitle.value="";el.customObjective.value="";el.customNotes.value="";el.customSlides.value="15";el.customWhatsappConfirmed.checked=false;await loadMyRequests();updateCustomWhatsApp();return}const{error}=await db.rpc("premium_submit_custom_presentation_request",{p_title:request.title,p_category:request.category,p_stage:request.stage,p_audience_gender:request.gender,p_objective:request.objective,p_desired_slides:request.slides,p_delivery_notes:request.notes||null});if(error)throw error;showStatus(el.customRequestStatus,"تم تسجيل الطلب. سيُراجع النطاق ويُحدد السعر عبر واتساب.");toast("تم إرسال طلب التصميم");el.customTitle.value="";el.customObjective.value="";el.customNotes.value="";el.customSlides.value="15";el.customWhatsappConfirmed.checked=false;await loadMyRequests();updateCustomWhatsApp()}catch(error){showStatus(el.customRequestStatus,error.message||"تعذر إرسال الطلب.",true)}finally{updateCustomWhatsApp()}}
function bubblePresentationRequest(row){let d={};try{d=JSON.parse(row?.PresentationData||'{}')}catch(_e){}const id=window.MishkatBubbleStore?.idOf?.(row)||row?.id||'';return{id,title:row?.Title||d.title||'عرض خاص',stage:d.stage||'',audience_gender:d.gender||'',desired_slides:Number(row?.SlidesCount||d.slides||0),status:row?.Status||'requested',estimated_price_sar:null,created_at:row?.CreatedAt||row?.['Created Date']||'',_bubble:row}}
async function loadMyRequests(){if(SCHOOL_EDITION){try{const Store=window.MishkatBubbleStore,f=Store.schema?.fields?.guidancePresentation||{},t=Store.schema?.dataTypes?.guidancePresentation||'Guidance_Presentation',c=Store.ctx?.()||{},constraints=[];if(c.id)constraints.push({key:f.guide||'Guide',constraint_type:'equals',value:c.id});const rows=await Store.list(t,constraints);state.requests=(rows||[]).map(bubblePresentationRequest).filter(r=>r._bubble?.Status==='requested'||(()=>{try{return JSON.parse(r._bubble?.PresentationData||'{}')?.kind==='custom_request'}catch(_e){return false}})());renderMyRequests()}catch(error){console.error(error);el.myRequestsList.innerHTML=`<div class="status-box error">تعذر تحميل طلبات العروض.</div>`}return}const{data,error}=await db.from("premium_custom_presentation_requests").select("id,title,stage,audience_gender,desired_slides,status,estimated_price_sar,created_at").eq("user_id",state.user.id).order("created_at",{ascending:false});if(error){el.myRequestsList.innerHTML=`<div class="status-box error">${escapeHtml(error.message)}</div>`;return}state.requests=data||[];renderMyRequests()}
