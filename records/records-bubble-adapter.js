"use strict";
/*
 * Records -> Bubble adapter V1.0.15
...
(function(global){
  ...
  async function saveRecord(recordType,form={},meta={},existing={}){
    const spec=specs[recordType],st=store();if(!spec||!st)return{supported:false};
  ...
  async function deleteRecord(typeName,id){if(!typeName||!id||!store())return false;return store().remove(typeName,id);}
  global.MishkatRecordsBubbleAdapter={schema,specs,supported,saveRecord,deleteRecord};
})(window);
