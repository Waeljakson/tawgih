...
(function(global){
  ...
  async function warmDirectory(){
    try{await load();global.dispatchEvent(new CustomEvent("mishkat:directory-ready",{detail:getSnapshot()}));global.dispatchEvent(new CustomEvent("mishkat:school-context-changed",{detail:global.MishkatSchoolContext?.getContext?.()||{}}));}catch(error){console.warn("Mishkat directory warm-up failed",error);}
  }
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",()=>setTimeout(warmDirectory,0),{once:true});else setTimeout(warmDirectory,0);
})(window);
